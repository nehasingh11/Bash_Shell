...
#(only consider consecutive repetions). Display the count and the line, separated by a space. 
...
#So, if a line X is followed by case variants, the output should count all of them as the same 
#(but display only the form X in the second column).
